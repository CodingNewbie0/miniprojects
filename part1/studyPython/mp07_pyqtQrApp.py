...
class qtApp(QWidget):
    ...
    def btnQrGenClicked(self):
        ...
        if data == '':
            QMessageBox.warning(self, '경고!', '데이터를 입력하시요')
            return
        else:
            qr_image = qrcode.make(data) # pixmap() 함수 생성
            qr_image.save('./studyPython/site.png')
